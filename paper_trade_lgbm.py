#!/usr/bin/env python3
"""Paper trading script using Alpaca API with LightGBM strategy.
...
Usage:
    ...
    # 55 15 * * 1-5 cd /path/to/trade && python paper_trade_lgbm.py >> logs/trade_lgbm.log 2>&1
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"logs/paper_trade_lgbm_{datetime.now():%Y%m%d}.log"),
        ],
    )
...
def show_status(broker):
    ...
    print(f"\n{'='*60}")
    print(f"  LGBM PAPER TRADING STATUS")
    print(f"{'='*60}")
...
def run_rebalance(strategy, broker, config, dry_run=False):
    ...
    print(f"\n{'='*65}")
    print(f"  LGBM TARGET PORTFOLIO  |  Capital: ${capital:,.0f}")
    print(f"{'='*65}")
...
def main():
    parser = argparse.ArgumentParser(description="LightGBM paper trading with Alpaca")
    ...
    # Connect to Alpaca paper trading (uses ALPACA_LGBM_* env vars)
